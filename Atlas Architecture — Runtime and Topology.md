...
Chat-to-Work promotion has a deterministic continuity floor. A model may choose Work earlier when the owner has delegated a durable responsibility, but a resolved capability may also declare that ephemeral execution is unsafe. `CapabilityRuntime` enforces that resolved fact before an Action occurrence is created: a caller without `work_id` receives `RuntimeContinuityRequired` and the action is not dispatched. The host runtime resolves its own user-systemd identity from `ATLAS_SERVICE_UNIT` and/or a cgroup leaf whose `MainPID` is the Atlas process; disagreement fails construction and an unresolved identity fails closed for user-service restart. Restarting the resolved Atlas unit requires Work while restarting another known user service remains an ordinary direct action. This is continuity enforcement, not authority, and it does not alter policy resolution.

An `uncertain` direct occurrence is never replayed merely because Chat still owes an answer. Chat records the normalized capability/scope/payload identity of unresolved or continuity-refused actions and deterministically refuses an identical retry inside the same turn; consequence-free observations may still resolve the outcome when it is immediately observable. If a residual obligation must survive the turn, `work.create` adopts the existing uncertain occurrence as its first step after verifying owner, capability, normalized scope and payload identity. Chat injects the originating `conversation_id` and owner `turn_id`; the runtime derives a stable key from that turn plus the exact objective/step template, so an identical composition retry is idempotent while one owner turn may still create distinct Work items. Work remains the durable owner of the objective while Chat remains its conversational surface.

Work binds an Action occurrence to its step immediately after the canonical Action row is created and before the executor runs. Recovery can also reconstruct a missing step binding from the occurrence's durable `work_id`/`step_id`, so a process death between Action creation and step update cannot cause a blind replay. Self-restart evidence such as the predecessor systemd invocation id is persisted on the Action before dispatch and merged, never overwritten, during abandoned-execution recovery. Boot order is consequently `recover_executing` → reconcile self-restart against the successor invocation → reconcile Work → resume only recovery-touched Work marked for automatic continuation → reconcile interrupted Work-control occurrences. Terminal Chat-origin Work uses one deduplicated completion path during both normal execution and recovery. Terminal completion persistence is deterministic and network-free: the deduplicated Chat turn is written immediately from Work truth, with bounded final output retained as grounding. Model phrasing is a post-start enrichment step, never part of boot recovery or Work completion. A background task may upgrade that same turn in place only after a closed semantic verifier confirms every factual claim is grounded in bounded `work.detail()` evidence; failed generation or verification leaves the deterministic turn intact. Reporting failure is downstream of Work truth and can never turn completed Work into a failed execution occurrence. Self-restart dispatch is additionally rate-limited from persisted receipt timestamps with margin below the systemd start-limit budget; a transport 502 is never treated as runtime truth.
...
Enabled providers are attempted in runtime priority order. `ModelRequest.capability_id` identifies the semantic role but does not currently select a separate model tier; completion reporting and its grounding verifier therefore use the same configured priority order as other model calls. Transport/provider failure falls through to another enabled provider rather than changing Atlas semantics, and the failed provider is logged as an operational warning. Decrypted model credentials are passed directly from `CredentialStore` to the selected in-process adapter; they are never round-tripped through process-global environment variables.
...
`atlas_api` provides authenticated read routes and CSRF-protected mutation routes. Session authentication establishes the owner principal; it is not an authority decision by itself. Long-running synchronous runtime work — model turns, generic capability execution, Work execution, MCP refresh and provider verification — is dispatched through worker threads rather than executed on the asyncio event loop. Recovery can therefore finish and the API can begin serving without waiting on completion-report generation; report enrichment starts only from the post-start background loop.

Runtime identity is resolved once when the API process is constructed and exposed by `/api/health`, `/api/system` and `host.status.inspect` as the Git revision that process actually loaded. Companion is stamped with its Git revision at build time and surfaces a visible mismatch rather than allowing a newer UI and older API to masquerade as one deployment.

`chat.turn` planning calls are evidence-bearing even though they are not Action occurrences: final turn metadata records provider/model, attempt number, latency, provider finish reason when available, token metrics, parse mode and a typed planner error code. Empty, malformed or structurally invalid decisions are retried once; a second failure terminates truthfully with `planner_unavailable` and no silent conversion into an empty assistant reply.
...
A Chat POST transport failure is not treated as execution truth. On a recoverable 502/503/504 or connection loss, Companion refetches the durable conversation and Work state. Once the authenticated owner turn is visible, a Chat-origin Work item becomes the continuation identity: the transport error is cleared, the composer is usable again, and conversation polling continues until that Work's deduplicated completion turn is present. An intermediate assistant reply therefore cannot terminate restart recovery while its linked Work is still outstanding. A deterministic completion turn whose `completion_report.mode` is `deterministic_pending` also keeps the conversation watch alive until post-start enrichment either reaches `grounded_model_verified` or settles on `deterministic_fallback`, so the owner does not stop on the pre-enrichment placeholder.
