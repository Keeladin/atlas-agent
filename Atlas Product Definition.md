...
The model proposes when an objective deserves durable ownership, but it is not the sole continuity boundary. After resource resolution, `CapabilityRuntime` refuses an action that cannot safely remain owned by an ephemeral caller unless a real `work_id` is present. Self-restart is the canonical example: Atlas may discuss or inspect it in Chat, but the restart itself must belong to Work before dispatch. Runtime service identity must itself be grounded: configured and cgroup/MainPID-derived identity may agree, otherwise Atlas fails closed, and the resolved unit/source is visible in Runtime. An already-dispatched `uncertain` action is never replayed; identical in-turn retries are deterministically refused, and if an obligation remains, Work may adopt that exact occurrence after deterministic identity checks. Terminal Chat-origin Work persists one deduplicated deterministic completion turn whether completion occurred normally or during recovery. Any model-written summary is a post-start enrichment of that same turn, verified against recorded Work evidence before it replaces the deterministic text. A narrow grounded reporting generation may phrase durable step outputs, while reporting failure is recorded separately and never changes terminal Work truth.
