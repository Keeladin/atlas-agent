...
- a durable obligation ledger for authenticated owner commitments, with `pending/complete/partial/interrupted/failed` intake, Attention derivation, evidence-based resolution and detached Work servicing;
...
Authenticated owner turns enter durable persistence as `pending` before planning. A separate obligation-intake model pass enumerates owner-grounded commitments; boot converts inherited `pending` intake to retryable `interrupted`, while exhausted extraction becomes `failed`. Durable objectives are staged with explicit obligation bindings and cannot execute in the request path. The ASGI handoff middleware records transport provenance only after the final response body send returns; the detached Work loop derives `staged -> runnable` from complete intake, semantic turn completion, confirmed handoff and open backing obligations. Work status is servicing truth, not commitment truth: state-change obligations resolve only from authoritative Action/Evidence, and communication obligations resolve only after a verified owner-facing Chat turn is durably persisted. Startup validates every frozen forbidden-state rule and enters sticky quarantine on any violation; health/auth/diagnostics and narrow store-mediated repair remain reachable while normal API and background execution are disabled.
...
Chat transport recovery follows durable state rather than HTTP status: after a recoverable disconnect, Companion refetches the owner turn and any Chat-origin Work, clears the transient request error once durable ownership is found, and continues following the conversation through the deterministic completion turn until any pending completion-report enrichment has either verified and upgraded that same turn or settled on the deterministic fallback. The selected conversation also keeps polling any `deterministic_pending` completion report during ordinary non-restart Work, so an in-place report upgrade cannot be missed merely because the Work already reached terminal state.
