...
def _parser() -> argparse.ArgumentParser:
    ...
    morning = sub.add_parser(
        "morning",
        help="Run the existing Morning Workflow through TaskRuntime",
    )
    ...
    plan = sub.add_parser(
        "plan",
        help="Create a durable task and bounded capability plan",
    )
    ...
    plan.add_argument(
        "--authority",
        default="interpret",
        help="Task authority scope",
    )
    plan.add_argument(
        "--run",
        action="store_true",
        help="Immediately run ready steps after planning",
    )
    ...
    recover = sub.add_parser(
        "recover",
        help="Explicitly resolve executions left running by an interrupted process",
    )
...
def main() -> None:
    ...
    runtime = build_runtime(
        db_path=args.db,
        provider_config=args.providers,
    )
    store = runtime.store

    if args.command == "result":
        print(TaskPresenter(store).build(args.task_id).render_markdown())
        return

    if args.command == "cancel":
        task = store.set_task_status(args.task_id, "cancelled")
        store.create_checkpoint(task.id, reason="task cancelled from CLI")
    ...
    if args.command == "index-text":
        ...
        task = store.create_task(
            ...
            metadata={"interface": "cli", "workflow": "knowledge_ingest"},
        )
        request = store.put_artifact(
            ...
            payload={
                ...
                "source_uri": args.source_uri or str(source.resolve()),
                "text": text,
        ...
        _print_result(runtime.run_until_blocked(task.id))
        return
    ...
    if args.command == "search":
        task = store.create_task(
            ...
            metadata={"interface": "cli", "workflow": "knowledge_search"},
        )
        request = store.put_artifact(
            task.id,
        ...
        _print_result(runtime.run_until_blocked(task.id))
        print(TaskPresenter(store).build(task.id).render_markdown())
        return
    ...
    if args.command == "status":
        print(
            json.dumps(
                store.snapshot(
                    args.task_id,
                    include_artifact_payloads=args.payloads,
                ),
                ensure_ascii=False,
                indent=2,
                default=str,
    ...
    if args.command == "run":
        runtime.resume_blocked(args.task_id)
        _print_result(runtime.run_until_blocked(args.task_id))
        return

    if args.command == "plan":
        if runtime.model_router is None:
            raise SystemExit("--providers is required for planning")
        planning = runtime.capabilities.get("planning.general").spec
        manifest = [
            item
            for item in runtime.capabilities.manifest()
            if item["id"] != planning.id
        ]
        planner = TaskPlanner(
            store=store,
        ...
        task, plan = planner.plan_and_create(
            ...
            success_criteria=tuple(args.criterion),
            constraints=tuple(args.constraint),
            authority_scope=args.authority,
    ...
    if args.command == "morning":
        task = store.create_task(
            ...
            success_criteria=(
                "The frozen Morning Workflow produces a verified non-empty pack "
                "for the requested operational day.",
            ),
            authority_scope="read",
        ...
        input_artifact = store.put_artifact(
            ...
            payload={
                ...
                "corrections": (
                    str(Path(args.corrections)) if args.corrections else None
                ),
                "day": args.day,
            },
        )
        ...
        packs = [
            artifact
            for artifact in store.list_artifacts(task.id)
