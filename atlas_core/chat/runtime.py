                ...
                if cid == "web.read" and isinstance(occurrence.result, dict):
                    ...
                    if quality.get("status") in {"dynamic_suspected", "empty"}:
                        ...
                        try:
                            ...
                            if render_occurrence.status == "pending_confirmation":
                                text = render_occurrence.summary or "Confirmation is required before Atlas can render this page."
                                return self._finish_turn(
                                    conversation_id, message, principal_id, owner_turn, text,
                                    {"action": render_public, "requires_confirmation": True}, action=render_public,
                                    skip_capture=capture_done, defer_capture=defer_capture,
                                )
