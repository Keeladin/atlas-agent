...
@dataclass(frozen=True)
class WorkItem:
    work_id:str;objective:str;status:str;owner_principal_id:str;created_at:str;updated_at:str;metadata:dict[str,Any];display_ref:str|None=None;artifact_class:str|None=None;workflow_class:str|None=None
    def as_dict(self)->dict[str,Any]:return {"work_id":self.work_id,"display_ref":self.display_ref,"artifact_class":self.artifact_class,"workflow_class":self.workflow_class,"objective":self.objective,"status":self.status,"owner_principal_id":self.owner_principal_id,"created_at":self.created_at,"updated_at":self.updated_at,"metadata":self.metadata}
