...
export type WorkAdaptation = { adaptation_id: string; work_id: string; base_revision: number; new_revision: number; from_ordinal: number; change_intent: string; reason: string; unchanged_goal: string; expected_impact: string; before_steps?: WorkStep[]; after_steps?: WorkStep[]; created_at: string }
export type WorkItem = { work_id: string; display_ref?: string | null; artifact_class?: string | null; workflow_class?: string | null; objective: string; status: string; owner_principal_id: string; created_at: string; updated_at: string; metadata: Record<string, unknown>; source_cadence_id?: string | null; revision?: number; steps?: WorkStep[]; adaptations?: WorkAdaptation[] }
export type CadenceKind = 'work_template' | 'intake_sweep'
export type Cadence = { cadence_id: string; name: string; objective: string; schedule: Record<string, unknown>; steps: Array<Record<string, unknown>>; enabled: boolean; kind: CadenceKind; intake_root_id?: string | null; max_candidates?: number; last_result?: Record<string, unknown> | null; next_run_at?: string | null; last_run_at?: string | null; last_work_id?: string | null }
export type ChatFocus = { cadence_id?: string; work_id?: string; step_ordinal?: number }
