...
export type WorkItem = { work_id: string; objective: string; status: string; owner_principal_id: string; created_at: string; updated_at: string; metadata: Record<string, unknown>; steps?: WorkStep[] }
...
export type MCPServer = { server_id: string; display_name: string; kind: 'mcp' | 'n8n'; transport: string; url: string; enabled: boolean; credential_configured: boolean; timeout_sec: number; read_timeout_sec: number; last_error?: string | null; last_discovered_at?: string | null; discovered_tool_count: number }
