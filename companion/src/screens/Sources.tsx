...
export function Sources() {
  ...
  const setReview = useMutation({
    mutationFn: ({ entry, status }: { entry: SourceObservation; status: 'unreviewed' | 'reviewed' | 'approved' | 'rejected' }) =>
      api<{ action: ActionOccurrence }>('/api/capabilities/library.set_review/invoke', {
        method: 'POST', body: JSON.stringify({ input: { root_id: entry.source_ref.root_id, relative_path: entry.source_ref.relative_path, status } }),
      }),
    onSuccess: async ({ action }) => {
      if (action.status !== 'succeeded') throw new Error(action.error || `Review update ${action.status}.`)
      await qc.invalidateQueries({ queryKey: ['library-reviews'] })
    },
  })
  ...
  return <Workspace title="Sources" subtitle="External origins, managed custody, and the runtime state created from them." tabs={<SegmentedNav items={OPERATIONS_TABS} />}>
    <Panel title="Source intake">
      ...
      {listing ? <div className="source-browser">
        <div className="source-browser-head"><span>Name</span><span>Type</span><span>Size</span><span>{isCleanLibrary ? 'Review status' : 'Lifecycle'}</span></div>
        ...
        {listing.entries.map(entry => {
          ...
          const visibleState = isCleanLibrary ? (review?.status ?? 'unreviewed') : lifecycle
          return <button type="button" className={`source-entry ${selected?.source_ref.relative_path === entry.source_ref.relative_path ? 'selected' : ''}`} key={`${entry.source_ref.relative_path}:${entry.observed_at}`} onClick={() => { if (isDir) open(entry.source_ref.relative_path); else { intake.reset(); setSelected(entry) } }} disabled={browse.isPending}>
            ...
            <span>{entry.object_type.replaceAll('_', ' ')}</span><span>{isDir ? '—' : formatBytes(entry.byte_size)}</span><span className={`chip ${visibleState === 'managed' || visibleState === 'approved' ? 'done' : ''}`}>{isDir ? 'browse' : visibleState}</span>
          </button>
        })}
    ...
    {selected ? <Panel title="Artifact lifecycle">
      <div className="artifact-lifecycle-head">
        ...
        <div className="actions">
          {isCleanLibrary ? <span className={`chip ${selectedReview?.status === 'approved' ? 'done' : ''}`}>Review: {selectedReview?.status ?? 'unreviewed'}</span> : <span className={`chip ${managedArtifact ? 'done' : ''}`}>{managedArtifact ? 'managed' : selectedArtifact ? 'established' : 'observed'}</span>}
          ...
          {isCleanLibrary ? <>
            <button type="button" disabled={setReview.isPending} onClick={() => setReview.mutate({ entry: selected, status: 'reviewed' })}>Mark reviewed</button>
            <button className="confirm" type="button" disabled={setReview.isPending} onClick={() => setReview.mutate({ entry: selected, status: 'approved' })}>Approve</button>
            <button className="danger" type="button" disabled={setReview.isPending} onClick={() => setReview.mutate({ entry: selected, status: 'rejected' })}>Reject</button>
            {selectedReview ? <button type="button" disabled={setReview.isPending} onClick={() => setReview.mutate({ entry: selected, status: 'unreviewed' })}>Reset review</button> : null}
          </> : null}
          <button className="primary" type="button" disabled={intake.isPending || Boolean(linkedWork.length) || (isCleanLibrary && selectedReview?.status !== 'approved')} onClick={() => intake.mutate(selected)}>
            {intake.isPending ? 'Processing…' : linkedWork.length ? 'Intake complete' : isCleanLibrary && selectedReview?.status !== 'approved' ? 'Approve before intake' : managedArtifact ? 'Retry routing' : 'Start intake'}
          </button>
        </div>
      </div>
      {setReview.isError ? <p className="offline-banner">Review update failed: {setReview.error.message}</p> : null}
      {isCleanLibrary ? <p className="meta">Review status records your vetting decision. Runtime file state remains separate. Only approved clean-library files can be started for intake here.</p> : null}
