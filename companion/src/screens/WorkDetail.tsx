...
function stateClass(status: string) {
  ...
  if (status === 'waiting_confirmation' || status === 'waiting' || status === 'paused') return 'confirm'
  if (status === 'failed' || status === 'cancelled') return 'failed'
  return ''
}

function ResponsibilityState({ item, completed, total }: { item: WorkItem; completed: number; total: number }) {
  return <Panel className="work-state-panel" title="Responsibility state">
...
export function WorkDetail() {
  ...
  const needsRetry = item?.status === 'paused' && (item.steps ?? []).some(step => step.status === 'waiting' && Boolean(step.error))
  ...
  return <Workspace
    ...
    headerActions={<><button className={needsRetry ? 'confirm' : ''} disabled={!item || terminal || mutate.isPending} onClick={() => mutate.mutate('resume')}>{needsRetry ? 'Retry' : 'Resume'}</button><button disabled={!item || terminal || item?.status === 'paused' || mutate.isPending} onClick={() => mutate.mutate('pause')}>Pause</button><button className="danger" disabled={!item || terminal || mutate.isPending} onClick={() => mutate.mutate('cancel')}>Cancel</button></>}
  >
    <div className="work-operator-stack">
      ...
      {item ? phone ? <details className="work-state-disclosure">
        <summary><span>Responsibility details</span><small>{completed}/{total} steps · {item.status.replaceAll('_', ' ')}</small></summary>
        <ResponsibilityState item={item} completed={completed} total={total} />
      </details> : <ResponsibilityState item={item} completed={completed} total={total} /> : null}
