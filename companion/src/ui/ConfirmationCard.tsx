...
export function ConfirmationCard({ item, onDone, onResolved }: { item: ActionOccurrence; onDone: () => Promise<unknown>; onResolved?: (item: ActionOccurrence) => void }) {
  ...
  return <Panel title="Confirm exact action" tone="decision-confirm">
    <p style={{ marginTop: 0 }}>{current.summary || current.capability_id}</p>
    ...
    {current.capability_id === 'memory.purge' ? <div className="memory-purge-guarantee">
    ...
    {current.status === 'pending_confirmation' ? <div className="actions">
    ...
    </div> : <div className="actions"><span className={`chip ${current.status === 'succeeded' ? 'done' : current.status === 'failed' || current.status === 'cancelled' ? 'failed' : 'running'}`}>{current.status}</span></div>}
  </Panel>
}
